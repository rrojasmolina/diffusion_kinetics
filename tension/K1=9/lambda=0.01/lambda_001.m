...
%tension is small but finite. These sumaltions are more challeging that the
%ones where the tension is zero, and a small numerical residual is obtained
...
global a epsilon  x1 alphat sigma2 a_0 area C_0_ lambda_ A1
            ...
            solinit = bvpinit(x1,@mat6init_tension,c1); %compute the initial guess used by bvp4c
            options = bvpset('RelTol',1e-5,'AbsTol',1e-5,'Nmax',1000);
            sol = bvp4c(@ACodefun,@ACbcfun,solinit,options); %find the shape equations for the initial protein density
            
            %Initial Gaussian protein density
            ...
            for l=1:t_steps
                ...
                K1=K1_.*heaviside(H-H_0); %curvature-dependent protein recruitment
                ...
                %compute the entries of the vector b in Ax=b
                for m=1:size(x1,2) 
                ...
                C=C_0_*alphat; %spontaneous curvature  
                ...
                %value of lambda
                save(sprintf('./lambda=0.01/min7_%d.txt',l)','x1', 'alphat', 'phi2','r2','A2','z2','M2', 'U2','T2', 'Q2','t','-ascii')
